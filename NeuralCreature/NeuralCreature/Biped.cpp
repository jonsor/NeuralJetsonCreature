...
{
	//Limbs:
	hips = new Box(glm::vec3(m_startPosition.x, m_startPosition.y, m_startPosition.z), glm::vec3(0.9f, 0.1f, 0.1f), 1.5f, 2.0f, 0.4f, 25);
...
double Biped::get2DAngle(Box* Box1, Box* Box2) {
	return 0.0;
}
...
void Biped::setAllTargetVelocities(std::vector<double>& resultVec)
{
	...
	//LEFT HIP
	getHips()->getdofConstraint("leftHip")->getRotationalLimitMotor(0)->m_targetVelocity = resultVec[0] * m;
	getHips()->getdofConstraint("leftHip")->getRotationalLimitMotor(1)->m_targetVelocity = resultVec[1] * m;
	getHips()->getdofConstraint("leftHip")->getRotationalLimitMotor(2)->m_targetVelocity = resultVec[2] * m;

	//RIGHT HIP
	getHips()->getdofConstraint("rightHip")->getRotationalLimitMotor(0)->m_targetVelocity = resultVec[3] * m;
	getHips()->getdofConstraint("rightHip")->getRotationalLimitMotor(1)->m_targetVelocity = resultVec[4] * m;
	getHips()->getdofConstraint("rightHip")->getRotationalLimitMotor(2)->m_targetVelocity = -resultVec[5] * m; //******* MINUS HER IKKE SIKKER OM DET VIRKER!! ********
...
void Biped::setNeuralNetwork(NeuralNetwork neuralNetwork)
{
	m_neuralNetwork = neuralNetwork;
}

NeuralNetwork Biped::getNeuralNetwork()
{
	return m_neuralNetwork;
}

NeuralNetwork* Biped::getNN()
{
	return &m_neuralNetwork;
}
...
{
	glm::vec3 pos = hips->getPosition();
	glm::vec3 prevPos = hips->getPreviousPosition();
